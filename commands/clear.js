...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if (target) {
            ...
            await channel.bulkDelete(filtered).then(msg => {
                res.setDescription(`Succesfully deleted ${msg.size} messages from ${target}.`);
                interaction.reply({ embeds: [res] }); // you can use ephemeral if you desire
            });
        } else {
            await channel.bulkDelete(amount, true).then(msg => {
                res.setDescription(`Succesfully deleted ${msg.size} messages from the channel.`);
